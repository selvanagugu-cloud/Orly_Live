...
- **Partitioned** on `snapshot_time` (hourly) — keeps query costs minimal
- **Clustered** on `icao24` and `origin_country` — fast filtering per airline
...
OpenSky API (every 10s)
  ...
  [Python consumer]         ← src/ingestion/consumer.py
        │
  BigQuery raw              ← paris_orly.raw_flights
  ...
  BigQuery marts
  ┌─────────────────────┐
  │ mart_live_board     │   ← current state per flight
  │ mart_airline_stats  │   ← delay stats per airline
  └─────────────────────┘
        │
